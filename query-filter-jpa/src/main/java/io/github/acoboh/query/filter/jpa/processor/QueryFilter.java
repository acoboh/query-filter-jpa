...
public class QueryFilter<E> implements Specification<E> {
	 ...
	 * <p>
	 * Only works with element. Otherwise will throw {@linkplain QFNotValuable}
	 * exception
	 *
	 * @param field
	 *            field of filter
	...
	public List<Pair<String, Direction>> getSortFieldWithFullPath() {
		...
		for (Pair<IDefinitionSortable, Direction> pair : list) {
			for (String path : pair.getFirst().getPathField()) {
	...
	public @Nullable Map<String, String> getActualJsonValue(String field) throws QFFieldNotFoundException {
		...
		if (qfSpec == null) {
			return null;
		} else if (qfSpec instanceof QFJsonElementMatch) {
			return ((QFJsonElementMatch) qfSpec).getMapValues();
		}
	...
	public Integer getActualCollectionValue(String field) throws QFFieldNotFoundException {
		...
		if (qfSpec == null) {
			return null;
		} else if (qfSpec instanceof QFCollectionMatch) {
			return ((QFCollectionMatch) qfSpec).getValue();
		}
